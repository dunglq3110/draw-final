...
!(function (a) {
		...
		return function (c, d) {
			...
			if (arguments.length) {
				if ("number" == typeof c && (a(c), (b.length = 0), d && d.length))
					for (e = 0; e < d.length; e += 1) b.push(d[e] + "Key");
				return this;
			}
	...
	function f(a) {
		...
		var b,
			...
			F = "character";
		if (
			(!D.nodeName || "textarea" === D.nodeName.toLowerCase()) &&
			(E === q || E === r || (13 === E && u))
		) {
			if (((v = !1), (f = D.value), (k = D.scrollTop), "number" == typeof D.selectionStart))
				...
				(g = o.selection.createRange()),
					...
					(l = s - t.length),
					n > 1
						? ((i = f.slice(0, l).split(m).length - 1), (j = t.split(m).length - 1))
						: (i = j = 0);
			}
			if (E === q || E === r)
				if (((b = p), (e = b.length), (y = 0), (z = 0), (A = 0), l !== s && -1 !== t.indexOf("\n")))
					if (
						...
						(y = 1),
							...
							g
								? (g.collapse(),
								  g.moveEnd(F, s + y * e - j - i),
								  g.moveStart(F, l + e - i),
								  g.select())
								: ((D.selectionStart = l + e), (D.selectionEnd = s + y * e), (D.scrollTop = k));
					else {
						if (!d(E, a)) return;
						0 === f.slice(w).indexOf(b) && (w === l ? (t = t.slice(e)) : (A = e), (z = e)),
							(D.value =
								f.slice(0, w) +
								f.slice(w + A, l) +
							...
							g
								? (g.collapse(),
								  g.moveEnd(F, s - z - y * e - j - i),
								  g.moveStart(F, l - A - i),
								  g.select())
								: ((D.selectionStart = l - A), (D.selectionEnd = s - z - y * e));
					}
				else if (c(E, a))
					g
						? ((g.text = b), g.select())
						: ((D.value = f.slice(0, l) + b + f.slice(s)),
						  (D.selectionEnd = D.selectionStart = l + e),
						  (D.scrollTop = k));
				else {
					if (!d(E, a)) return;
					0 === f.slice(l - e).indexOf(b) &&
						((D.value = f.slice(0, l - e) + f.slice(l)),
						g
							? (g.move(F, l - e - i), g.select())
							: ((D.selectionEnd = D.selectionStart = l - e), (D.scrollTop = k)));
				}
			else if (u) {
				if (0 === l || "\n" === f.charAt(l - 1)) return void (v = !0);
				if (
					((w = f.lastIndexOf("\n", l - 1) + 1),
					(x = f.indexOf("\n", l)),
					-1 === x && (x = f.length),
					(B = f.slice(w, x).match(/^[ \t]*/)[0]),
					(C = B.length),
					w + C > l)
				)
					return void (v = !0);
				g
					? ((g.text = "\n" + B), g.select())
					: ((D.value = f.slice(0, l) + "\n" + B + f.slice(s)),
					  (D.selectionEnd = D.selectionStart = l + n + C),
					  (D.scrollTop = k));
			}
	...
	(l = i([
		...
		(a.handlers = { keydown: f, keypress: g }),
		(a.addExtension = function (a, b) {
			return (
				a && "string" == typeof a && "function" == typeof b && (x[a] || (x[a] = []), x[a].push(b)),
				this
			);
		}),
		(a.set = function (a, b) {
			...
			if (a)
				for (
					c = arguments.length < 2 || b,
						d = a,
						e = d.length,
						"number" != typeof e && ((d = [d]), (e = 1)),
						c ? ((f = j), (g = "true")) : ((f = k), (g = "")),
						i = 0;
					e > i;
					i += 1
					...
					(l = d[i]),
						l &&
							...
							(h("set", [l, c]), l.setAttribute("data-taboverride-enabled", g), f(l));
			return this;
		}),
		(a.tabSize = function (a) {
			var b;
			if (arguments.length) {
				if (a && "number" == typeof a && a > 0) for (p = "", b = 0; a > b; b += 1) p += " ";
				else p = "	";
				return this;
			}
			return "	" === p ? 0 : p.length;
...
!(function (a) {
	...
	"object" == typeof exports && "function" == typeof require
		? a(require("jquery"), require("taboverride"))
		: "function" == typeof define && define.amd
		? define(["jquery", "taboverride"], a)
		: a(jQuery, tabOverride);
})(function (a, b) {
	"use strict";
	function c(a, c) {
		a.off(
			{ "keydown.tabOverride": b.handlers.keydown, "keypress.tabOverride": b.handlers.keypress },
			c
		);
	}
	function d(a, d) {
	...
	function e(a, d) {
		b.utils.executeExtensions("addDelegatedListeners", [a, d]),
			c(a, d),
			a.on(
				{ "keydown.tabOverride": b.handlers.keydown, "keypress.tabOverride": b.handlers.keypress },
				d
	...
	(f = a.fn.tabOverride =
		function (a, c) {
			var f,
				g = !arguments.length || a,
				h = "string" == typeof c;
			return (
				h
					? ((f = this),
					  b.utils.executeExtensions("setDelegated", [f, c, a]),
					  g ? e(f, c) : d(f, c))
					: b.set(this, g),
				this
			);
		}),
		(f.utils = { addDelegatedListeners: e, removeDelegatedListeners: d }),
		(f.tabSize = b.tabSize),
		(f.autoIndent = b.autoIndent),
		(f.tabKey = b.tabKey),
		(f.untabKey = b.untabKey);
});
